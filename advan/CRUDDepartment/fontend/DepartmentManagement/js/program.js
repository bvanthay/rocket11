$(function () {
    ...
    $(".header").load("header.html", function () {
        ...
        // chỗ này để khi mà thằng user vào trang department thì nó sẽ không thể xem được listdepartment
        if (storage.getItem("ROLE") == "User" ) {
            document.getElementById("viewlistdepartment").style.display = "none"
        }
    });
    $(".main").load("home.html");
...
function logout() {
    //lưu bao nhiêu thằng thì xóa bây nhiêu thằng 
    storage.removeItem("ID");
    storage.removeItem("FULL_NAME");
    storage.removeItem("USERNAME");
    storage.removeItem("PASSWORD");

    //sau khi xóa xong sẽ cho nó quay lại trang login 
    window.location.replace("http://127.0.0.1:5500/html/login.html");
}
...
function getListDepartments() {
    ...
    $.ajax({
        ...
        error(jqXHR, textStatus, errorThrown) {
            // cái này là để khi báo lỗi 403 thì nó sẽ hiện ra trang forbidden 
            if (jqXHR.status == 403 ) {
    ...
    $.ajax({
        ...
        success: function (data, textStatus, xhr) {
            if (data) {
                ...
                var department = {
                    name: name,
                    //để khi add 1 thằng mới nó sẽ lấy theo tên của thằng  id
...
function deleteAllDepartment() {
    ...
    // open confirm kieu bạn có muốn xóa bản ghi 
